...
class LawOfNatureSpec extends FlatSpecLike with Matchers {
  "A living Cell" should "die on under population" in {
    ...
    val result = LawsOfNature.valueAt(LivingCell(1, 1), new World(() => (1, 1), seed = Some(seed)))
  ...
  "A living Cell" should "die on overcrowding" in {
    ...
    val result = LawsOfNature.valueAt(LivingCell(1, 1), new World(() => (2, 2), seed = Some(seed)))
  ...
  "Out of nothing" should "be giving birth to a new cell" in {
    ...
    val result = LawsOfNature.valueAt(Nothing(1, 1), new World(() => (1, 1), seed = Some(seed)))
